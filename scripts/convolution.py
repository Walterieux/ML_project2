...
def apply_patches_for_array_of_images(image_list, rgb_binary=True):
    ...
    for number, image in enumerate(image_list):
        if rgb_binary == False:
            list_of_batches[:, :,
            number * nb_patch_per_image: (number + 1) * nb_patch_per_image] = create_patches_from_training_or_test(
                image, rgb_binary=False, )
        else:
...
def create_patches_from_training_or_test(image, rgb_binary=True):
    ...
    nb_elem_image = 330
    if rgb_binary == True:
        batches = np.zeros((input_size, input_size, nb_batches), dtype='uint8')
    else:
        batches = np.zeros((output_size, output_size, nb_batches), dtype='uint8')
