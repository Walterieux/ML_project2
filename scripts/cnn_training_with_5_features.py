# -*- coding: utf-8 -*-
"""
Created on Tue Dec  1 13:07:02 2020
...
def train_model(train_images, test_images, train_labels, test_labels):
    ...
    # create mini_patches
    patches_train_images = create_patches(train_images, (img_patch_size, img_patch_size, 5))
    patches_test_images = create_patches(test_images, (img_patch_size, img_patch_size, 5))
    ...
    model1.add(
        layers.Conv2D(64, kernel_size=(5, 5), activation='relu', padding='same',
                      input_shape=(img_patch_size, img_patch_size, 5)))
    model1.add(layers.Conv2D(128, kernel_size=(5, 5), activation='relu'))
    model1.add(layers.MaxPool2D((2, 2)))
    model1.add(Dropout(0.20))  # Avoid overfitting

    model1.add(layers.Conv2D(64, kernel_size=(5, 5), activation='relu', padding='same'))
    model1.add(layers.Conv2D(64, kernel_size=(5, 5), activation='relu'))
    model1.add(layers.MaxPool2D((2, 2)))
